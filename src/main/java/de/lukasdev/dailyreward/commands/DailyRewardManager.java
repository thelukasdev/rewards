...
public class DailyRewardManager implements CommandExecutor, Listener {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (!(sender instanceof Player)) {
            sender.sendMessage("§cDu musst ein Spieler sein, um diesen Befehl zu verwenden.");
            return true;
        }
        ...
        if (cmd.getName().equalsIgnoreCase("daily")) {
            if (canOpenDailyGUI(player)) {
                ...
                player.sendMessage(ChatColor.RED + "§7Du kannst deine Belohnung in §b"
                        + getTimeUntilNextOpen(player) + " §bStunden §7wieder öffnen.");
            }
        }
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (event.getView().getTitle().equals("§7Deine Belohnung")) {
    ...
    private void claimDailyReward(Player player) {
        ...
        if (random.nextInt(100) < 1) {
            ...
            Bukkit.broadcastMessage("§8»§7 Der Spieler §b" + player.getName() + " §7hat den §4§lJackpot §7im Daily Reward gewonnen!");
        }

        player.getInventory().addItem(reward);

        player.sendMessage("§aDu hast deine tägliche Belohnung erhalten.");
    }
